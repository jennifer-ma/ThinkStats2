...
#    def MakeModel(self):
    ...
    def RunModel(self):
        ...
        count = min(200, len(self.means))
        # dataset2.df, map
        # go thru keys in map, swap
        means = []
        
        for caseid in dataset2.map:
            
            count -= 1
            if count <= 0:
                break
        
            preg_indices = dataset2.map[caseid]
            swappedPregLngths = self._GetPregLengthsAndSwap(preg_indices, self.dataset2)
            
            # calc mean
            mean = self._GetMean(swappedPregLngths)
            # add mean to means
            means.append(mean)
